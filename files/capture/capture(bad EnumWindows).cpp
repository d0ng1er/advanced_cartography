...
// Source (next 2 funcs):
// https://www.purebasic.fr/english/viewtopic.php?f=13&t=29981&start=15
bool EnumWindowsPro(
        HWND hWnd,
        LPDWORD tpAddress )
{
    //cerr << "at callback function\n";
    DWORD testProc = 0;
...
/* Just to be clear: tempParam in the below function starts out as a 
 * process ID, then gets recast into a window-handle-cast-as-dword 
 * in enumwindowspro to make the compiler happy, then recast back into a window
 * handle and returned to complete the function. Is this terrible and
 * confusing? Yes. Doing it anyway.
 */
...
HWND GetProcHwnd()
{
    ...
    return (HWND)tempParam; // now an HWND, probably ok since it's all 32 bit
}
